...
def load_data_set():
    features_data = pd.read_csv('Brain_Integ_X.csv')
    labels_data = pd.read_csv('Brain_Integ_Y.csv')

    time_of_death = np.asarray([t[0] for t in labels_data['Survival']])\
        .astype('float32')
    censored = np.asarray([c[0] for c in labels_data['Censored']])\
        .astype('int32')
...
# tf.Graph input
x = tf.placeholder("float", [None, None], name='features')  # Set to [, None]
# to definite value
...
with tf.Session() as sess:
    ...
    for epoch in range(TRAINING_EPOCHS + 1):
        ...
        for i in range(total_batch - 1):    # Loop over all batches
            ...
            _, batch_cost, batch_pred = sess.run([optimizer, cost, pred],
                                                 feed_dict={x: batch_x,
                                                            y: batch_y})
    ...
    # TESTING PHASE #
    _, test_predicted_values = sess.run([cost, pred], feed_dict={x: x_test,
                                                                 y: y_test})
